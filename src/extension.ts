        ...
        return `<!DOCTYPE html><html><head>
			<style>
				...
				* { box-sizing: border-box; }
				...
				.pred-sec .grid-container { height: 58px; overflow-y: auto; padding: 4px 8px; }
				.vault-sec { flex: 1; display: flex; flex-direction: column; overflow: hidden; min-height: 150px; }
				.vault-sec .grid-container { flex: 1; overflow-y: auto; padding: 4px 8px 16px 8px; }
				...
				.prompt-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; padding-bottom: 4px; }
				...
				.prompt-card.expanded { grid-column: span 2; grid-row: span 3; height: auto !important; min-height: 40px; border-color: var(--accent); background: var(--accent-dim); padding: 5px 6px; z-index: 10; cursor: default; justify-content: flex-start; gap: 4px; }
				...
				.card-actions { display: none; gap: 4px; flex-wrap: wrap; margin-top: auto; }
				...
				.tabs { display: flex; overflow-x: auto; gap: 4px; padding: 0 4px; flex: 1; font-size: 11px; scrollbar-width: none; align-items: center; height: 100%; }
				.tabs::-webkit-scrollbar { display: none; }
				.tab { padding: 2px 8px; cursor: pointer; opacity: 0.5; border-radius: 3px; white-space: nowrap; height: 20px; line-height: 14px; transition: 0.2s; border: 1px solid transparent; display: flex; align-items: center; }
				.tab:hover { opacity: 0.8; }
				.tab.active { opacity: 1; background: var(--accent-dim); color: var(--accent); border: 1px solid var(--accent); font-weight: 600; }
				
				.config-panel { display: none; background: var(--card-bg); border-bottom: 1px solid var(--card-border); padding: 4px 8px; flex-shrink: 0; flex-direction: column; gap: 4px; }
				...
				.collapsible-sec { padding: 8px 0; border-top: 1px solid var(--card-border); margin-top: 2px; }
				...
				<div class="vault-sec">
					<div class="vault-toolbar">
						...
						<div class="header-btns">
							<span class="header-btn" id="btn-ctrl" onclick="toggleCtrl()" title="Enable Selection Mode">☑️ SELECT</span>
							<span class="header-btn" onclick="vscode.postMessage({type:'addCat'})" title="Add Category">➕</span>
							<span class="header-btn" onclick="openGenModal()" title="AI Generate Prompts" style="opacity: 0.8; font-size: 10px;">✨ GENERATE</span>
						</div>
					</div>
			...
			<script nonce="${nonce}">
				...
				function render() {
                    ...
                    if (tabsEl) tabsEl.innerHTML = tabs.map(t => \`<div id="tab-\${t.id}" class="tab \${activeTab === t.id ? 'active' : ''}" onclick="activeTab='\${t.id}';render()">\${t.name}</div>\`).join('');
                ...
                document.addEventListener('keydown', (e) => {
                    ...
                    if(e.key === 'Tab') {
                        ...
                        let cur = Array.from(tabs).findIndex(t => t.classList.contains('active'));
                        let next = (cur + 1) % tabs.length;
                        tabs[next].click();
                        tabs[next].scrollIntoView({ inline: 'center' });
                        e.preventDefault();
                    }
                });
